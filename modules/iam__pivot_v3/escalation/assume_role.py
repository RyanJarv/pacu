...
def assume_role(source: Node, dest: Node) -> Optional[Escalation]:
    ...
    if sim_result == ResourcePolicyEvalResult.DENY_MATCH:
        return None   # Node was explicitly denied from assuming the role

    if sim_result == ResourcePolicyEvalResult.NO_MATCH:
        return None  # Resource policy must match for sts:AssumeRole, even in same-account scenarios
